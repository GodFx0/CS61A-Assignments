...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    ones_num = player_score % 10
    tens_num = opponent_score // 10 % 10
    return max(abs(3 * abs(ones_num - tens_num)), 1)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    old_strategy = strategy(0,0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != old_strategy:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while scoring_num <= 9:
        scoring_num += 1
        avg_score = average_dice(scoring_num, dice)
        if avg_score > max_scoring:
            max_scoring_num = scoring_num
