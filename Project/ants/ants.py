...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        dis = 0
        while (not place.entrance.is_hive) and (dis < self.lower_bound or len(place.bees) == 0) and dis < self.upper_bound :
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if self.ant_contained == None and other and other.is_container == False:
            return True
        else:
            return False
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while cur_place.exit is not None:
